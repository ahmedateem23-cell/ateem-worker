  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/telegram-order") {
      ...
      try {
        ...
        const paymentLabel =
          order.payment_method === "bankak" ? "بنكك"
          : order.payment_method === "fawry" ? "فوري"
          : order.payment_method === "cod" ? "عند الاستلام"
          : "غير محدد";
        const text =
          "⚠️ لم يُنشأ الطلب تلقائيًا في Odoo — راجعه يدويًا\n" +
          (order.debug_reason ? "🔍 السبب: " + order.debug_reason + "\n" : "") +
      ...
      try {
        ...
        if (tmpl.product_variant_ids && tmpl.product_variant_ids.length) {
          const variantRecords = await callKw("product.product", "read", [
            ...
            // virtual_available (مش qty_available) عمدًا: ده الرقم اللي
            // بياخد في الحسبان الحجوزات بتاعة أي أوردر اتأكد (action_confirm)
            // حتى لو لسه ما خرجش فعليًا من المخزن. لو استخدمنا qty_available
            // العادي، طلبين مختلفين ممكن ياخدوا نفس آخر قطعة قبل ما حد
            // يعمل "تسليم" يدوي في Odoo — سيناريو overselling حقيقي.
            ["id", "lst_price", "virtual_available",
          ...
          variants = variantRecords.map((v) => {
            ...
            totalStock += Math.max(0, v.virtual_available || 0);
            return {
              id: v.id,
              values: valuesMap,
              price: v.lst_price,
              stock: Math.max(0, Math.floor(v.virtual_available || 0)),
        ...
        const product = {
          ...
          // بييجي من مجموع virtual_available لكل الفاريانتس (غالبًا واحد بس).
